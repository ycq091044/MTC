...
def myalgo(mask, T, C1, C2, P2, R, iteration):
    ...
    if (max(d1, d3, new_d1, new_d2) <= 2 * R) and (args.multi == 1) or args.multi == 0:
        ...
        A1_low, A2_low, A3_low, Q1_low, Q2_low, _, _ = myalgo(mask_, T_, C1_, C2_, P2_, R=R, iteration=20)
    ...
    for i in range(iteration): 
        ...
        # ALS
        A2 = optimize(A2.T, (A3.T@A3)*(A1.T@A1) + LAMBDA * (Q1.T@Q1)*(A3.T@A3), LAMBDA * np.einsum('ijk,ir,kr->rj',C1,Q1,A3,optimize=True) + \
    ...
    return A1, A2, A3, Q1, Q2, loss_list, rec_list
...
if __name__ == '__main__':
    ...
    for j in range(1):
        ...
        A1, A2, A3, Q1, Q2, loss, rec = myalgo(mask, T, C1, C2, P2, R=10, iteration=100)
        # pickle.dump([A1, A2, A3, Q1, Q2, loss, rec], open('synthetic_result/X_C1_C2_known_{}_jacobi_{}_multi_{}.pkl'.format(args.jacobi, args.multi, j), 'wb'))
