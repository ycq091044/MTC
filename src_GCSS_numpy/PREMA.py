...
def CPD(T, R, iteration, A1, A2, A3):
    ...
    for i in range(iteration):
        ...
        A1 = multigrid2(A1.T, (A3.T@A3)*(A2.T@A2), (T_1 @ la.khatri_rao(A2, A3)).T).T

        A2 = multigrid2(A2.T, (A3.T@A3)*(A1.T@A1), (T_2 @ la.khatri_rao(A1, A3)).T).T
        
        tmp = la.khatri_rao(A1, A2)
        A3 = multigrid2(A3.T, (A2.T@A2)*(A1.T@A1), (T_3 @ tmp).T).T
        
        rec = (tmp @ A3.T).T
        recLOSS = la.norm(rec-T_3)
...
def TendiB_init(T, O1, O2, R):
    ...
    A1, tmp2, A3 = CPD(O2, R, 60, A1, tmp2, A3)
...
def GD(iteration, X, O1, O2, mask, R):
    ...
    # tmp1 = np.random.random((new_d1, R))
    # tmp2 = np.random.random((new_d2, R))
    A1, A2, A3, tmp1, tmp2 = TendiB_init(X, O1, O2, R)
    ...
    def grad_tmp1(Lambda):
        return - np.einsum('ijk,jr,kr->ir',O1,A2,A3,optimize=True) + \
                    np.einsum('ar,br,bj,cr,cj->aj',tmp1,A2,A2,A3,A3,optimize=True) + \
                    beta * np.ones((new_d1, d1)) @ A1
        # return - O1_1 @ la.khatri_rao(A2, A3) + tmp1 @ ((A2.T @ A2) * (A3.T @ A3))
    
    def grad_tmp2(Lambda):
        return np.einsum('ijk,ir,kr->jr',O2,A1,A3,optimize=True) + \
                    np.einsum('ar,br,bj,cr,cj->aj',tmp2,A1,A1,A3,A3,optimize=True) + \
                    beta * np.ones((new_d2, d2)) @ A2
    ...
    def grad_A1(Lambda):
        return - Lambda * np.einsum('ijk,jr,kr->ir',O2,tmp2,A3,optimize=True) + Lambda *  np.einsum('ar,br,bj,cr,cj->aj',A1,tmp2,tmp2,A3,A3,optimize=True)\
                            + (M1 * (A1 @ la.khatri_rao(A2, A3).T - X_1)) @ la.khatri_rao(A2, A3) + beta * np.ones((d1, new_d1)) @ tmp1
    ...
    def grad_A2(Lambda):
        return - Lambda * np.einsum('ijk,ir,kr->jr',O1,tmp1,A3,optimize=True) + Lambda *  np.einsum('ar,br,bj,cr,cj->aj',A2,tmp1,tmp1,A3,A3,optimize=True)\
                            + (M2 * (A2 @ la.khatri_rao(A1, A3).T - X_2)) @ la.khatri_rao(A1, A3) + beta * np.ones((d2, new_d2)) @ tmp2
    ...
    def line_search(obj_func, X, coord, grad, alpha = [1e-9, 3e-8], visit = [-1, -1], depth=3):
...
if __name__ == '__main__':
    ...
    for j in range(1):
        ...
        # pickle.dump([A1, A2, A3, tmp1, tmp2, loss, rec], open('result_google/PREMA_{}_{}.pkl'.format(args.partial, j), 'wb'))
